...
class Palette(BaseModel):
    ...
    def save_current_results(self):
        ...
        for idx in range(self.batch_size):
            ...
            name = self.path[idx].split('/')[-1]

            ret_path.append('Out_{}'.format(name))
            # ret_result.append(self.visuals[idx-self.batch_size].detach().float().cpu())

            view =torch.cat([self.visuals[idx::self.batch_size][0:1
                   ],self.visuals[idx::self.batch_size][-1:],
                                     self.gt_image[idx].unsqueeze(0)],0
                  ) # prompt / output
            ret_result.append(view.detach().float().cpu())


            if 0:
                ret_path.append('prompt_{}'.format(name))
                ret_result.append(self.cond_image[idx].detach().float().cpu())

                ret_path.append('GT_{}'.format(name))
                ret_result.append(self.gt_image[idx].detach().float().cpu())

                ret_path.append('Process_{}'.format(name))
                ret_result.append(self.visuals[idx::self.batch_size].detach().float().cpu())

            # ret_path.append('Out_{}'.format(self.path[idx].split('/')[-1]))
            # ret_result.append(self.visuals[idx - self.batch_size].detach().float().cpu())
    ...
    def train_step(self):
        ...
        for train_data in tqdm.tqdm(self.phase_loader):
            ...
            self.optG.zero_grad()
            loss = self.netG(self.gt_image, self.cond_image, mask=self.mask)  # mask: None
            loss.backward()
            self.optG.step()
