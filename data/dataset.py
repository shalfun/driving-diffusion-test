...
class GenerateDatasetMultiView(data.Dataset):
    ...
    def get_instance(self, index):
        ...
        if 0:  # origin list
            ...
            # img_path = os.path.join(self.image_dir, item[1])  # one frame
            # im = cv2.imread(img_path)
            for img_name in item:
                img_path = os.path.join(self.image_dir, img_name)
                img_paths.append(img_path)

            imgs = [Image.open(img_path).convert('RGB') for img_path in img_paths]  # 0: fl  1:f  2:fr
            imgs = [self.tfs(img) for img in imgs]
        ...
        #####LOOP PREPARE#####
        view_num = 3
        black_image = np.zeros((4 * view_num, self.image_size[0], self.image_size[1]))  # 4 means element number
        ...
        for idx_cam, cam_name in enumerate(['spherical_left_forward', 'onsemi_obstacle', 'spherical_right_forward']):
            extrinsics = np.array(np.asarray(trans_server.get_static_transform('vehicle', cam_name)[1][:3]),
                                  dtype=np.float32)
            intrin_param = trans_server.intrinsics_dict[cam_name][0]  # disgarding D & image_shape
            ...
            for idx_ele, name in enumerate(name_list):  # road_seg/SEG_xxx

                # if idx_ele in [1,2,3]: continue
                cur_cls_mat_path = os.path.join(self.json_path, '%s/road_seg/' % path,
                                                '%s_%s.npz' % (idx, seg_list[idx_ele]))
                seg_dict[name] = np.load(cur_cls_mat_path)['arr_0']
                reg_dict[name] = np.load(os.path.join(self.json_path, '{}/{}'.format(path, reg_list[idx_ele]),
                                                      '%s_REG.npz' % idx))['arr_0']
                ...
                for pt in gt_reg:
                    ...
                    try:
                        ...
                        w = int(pt2d[0] * s_w)
                        if h > 0 and w > 0:
                            black_image[idx_cam * view_num + idx_ele, int(pt2d[1] * s_h), int(pt2d[0] * s_w)] = prompt_value_list[idx_ele]
                    except:
                        pass
...
class SuperResolutionDataset(data.Dataset):
    def __init__(self, data_root, data_flist, data_len=-1, data_path='', image_size=[224, 224], loader=pil_loader,
        ...
        multipy = 2
        self.tfs_clear = transforms.Compose([
            transforms.Resize((image_size[0]*multipy, image_size[1]*multipy)),  # todolxf
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
        ])
        self.tfs_fuzzy = transforms.Compose([
            transforms.Resize((image_size[0], image_size[1])),
            transforms.Resize((image_size[0]*multipy, image_size[1]*multipy)),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
        ])
        self.loader = loader
    ...
    def get_instance(self, index):
        ...
        # im = cv2.imread(img_path)
        img = Image.open(img_path).convert('RGB')  # 0: fl  1:f  2:fr
        img_c = self.tfs_clear(img.copy())
        img_f = self.tfs_fuzzy(img.copy())



        #######################################3
        ret['gt_image'] = img_c
        ret['cond_image'] = img_f # cond_image
        ret['path'] = img_path  # file_name
        return ret
